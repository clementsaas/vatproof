...
# Import des services depuis le package principal
from app.services import VATService, FileService
...
def api_download_batch_zip(batch_id):
    ...
    try:
        ...
        # Vérification que le lot existe et est terminé
        batch_info = job_storage.get_batch(batch_id)
        if not batch_info:
            return jsonify({'error': 'Lot non trouvé'}), 404
        
        if batch_info['status'] != 'completed':
            return jsonify({'error': 'Lot non terminé'}), 400
        
        # Récupération des jobs terminés avec succès
        jobs = job_storage.get_batch_jobs(batch_id)
        completed_jobs = [job for job in jobs 
                         if job['status'] == 'completed' 
                         and job.get('result', {}).get('is_valid')]
        ...
        zip_result = zip_service.create_batch_zip(batch_id, completed_jobs)
        
        if not zip_result['success']:
            return jsonify({'error': zip_result['error']}), 500
        
        current_app.logger.info(
            f"ZIP généré pour le lot {batch_id}: "
            f"{zip_result['files_count']} fichiers, "
            f"{zip_result['total_size']} bytes"
        )
        ...
        return send_file(
            zip_result['zip_path'],
            as_attachment=True,
            download_name=zip_result['zip_filename'],
            mimetype='application/zip'
        )
        
    except Exception as e:
        current_app.logger.error(f"Erreur téléchargement ZIP {batch_id}: {str(e)}")
        return jsonify({
...
def api_batch_zip_info(batch_id):
    ...
    try:
        ...
        jobs = job_storage.get_batch_jobs(batch_id)
        pdf_jobs = [job for job in jobs 
                   if job['status'] == 'completed' 
                   and job.get('result', {}).get('is_valid')
                   and job.get('result', {}).get('pdf_path')]
        
        return jsonify({
            'batch_id': batch_id,
            'status': batch_info['status'],
            'zip_available': len(pdf_jobs) > 0,
            'pdf_count': len(pdf_jobs),
            'valid_jobs': len([j for j in jobs if j.get('result', {}).get('is_valid')]),
            'completed_jobs': len([j for j in jobs if j['status'] == 'completed']),
            'total_jobs': len(jobs)
        })
        
    except Exception as e:
        current_app.logger.error(f"Erreur info ZIP {batch_id}: {str(e)}")
